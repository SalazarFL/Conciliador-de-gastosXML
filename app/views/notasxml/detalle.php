...
<div class="card">
    ...
    <div style="padding:18px;display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:14px;font-size:13px;">
    ...
    <div style="padding:0 18px 18px;display:flex;gap:8px;"><a class="btn btn-primary" target="_blank" href="<?= $baseUrl ?>/documentos/xml/<?= (int)$n['id'] ?>"><i class="fas fa-code"></i> Visualizar XML</a><?php if (!empty($n['ruta_pdf']) && is_file($n['ruta_pdf'])): ?><a class="btn btn-outline" target="_blank" href="<?= $baseUrl ?>/documentos/pdf/<?= (int)$n['id'] ?>"><i class="fas fa-file-pdf"></i> Visualizar PDF</a><?php endif; ?></div>
</div>
